                ...
                try {
                  if ($.layer.arrayIndexOf($.layer.pictureType, xml.file.toString().split('.').slice(-1)) !== -1) {
                    ...
                    if (!genFilePath.exists || genFilePath.exists && genFilePath.length !== waitToWrite.length) {
                      ...
                      if (!genFilePath.parent.exists) { genFilePath = File($.layer.tempFolder.toString() + $.layer.slash + decodeURIComponent(file.name.toString())) }
                    ...
                    if (!genFilePath.exists || genFilePath.exists && genFilePath.length !== waitToWrite.length) {
                      if (!genFilePath.parent.exists) { genFilePath.create() }
                      if (!genFilePath.parent.exists) { genFilePath = File($.layer.tempFolder + $.layer.slash + decodeURIComponent(file.name.toString())) }
                      genFilePath.open('w')
                      genFilePath.encoding = 'BINARY'
                      genFilePath.write(waitToWrite)
                      genFilePath.close()
