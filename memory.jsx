...
(function(global){
    ...
    sp.xmlFileNames.forEach(function(item,index){
            this.add("item",item);
        },droplist)
...
//~      sp.saveSetting ("language","ch");
    ...
    settingxml = new XML(sp.settingsFile.readd());
    sp.xmlFileNames.length = 0;
    sp.forEach(settingxml.ListItems,function(item,index){
                 sp.xmlFileNames.push(item.toString ());
        });
...
}catch(err){err.printa();err.princ()}
