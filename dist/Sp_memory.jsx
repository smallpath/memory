...
(function() {
  ...
  $.layer.extend($.layer.prototype, {
    ...
    toXML: function(elementName, helperObj) {
      ...
      if (elementArr.length === 0) { $.layer.forEach.call(layers, loopFunc) } else { $.layer.forEachLayer.call(layers, loopFunc) }
  ...
  $.layer.extend($.layer.prototype, {
    ...
    newLayer: function(xml, thisComp) {
      ...
      try {
        ...
        if (xml.outPoint !== 'undefined') { layer.outPoint = parseFloat(xml.outPoint) }
                ...
                try {
                  if ($.layer.arrayIndexOf($.layer.pictureType, xml.file.toString().split('.').slice(-1)) !== -1) {
                    ...
                    if (!genFilePath.exists || genFilePath.exists && genFilePath.length !== waitToWrite.length) {
                      if (!genFilePath.parent.exists) { genFilePath.create() }
                      if (!genFilePath.parent.exists) { genFilePath = File($.layer.tempFolder.toString() + $.layer.slash + decodeURIComponent(File(xml.file.toString()).name.toString())) }
                    ...
                    if (!genFilePath.exists || genFilePath.exists && genFilePath.length !== waitToWrite.length) {
                      if (!genFilePath.parent.exists) { genFilePath.create() }
                      if (!genFilePath.parent.exists) { genFilePath = File($.layer.tempFolder + $.layer.slash + decodeURIComponent(File(xml.file.toString()).name.toString())) }
                      genFilePath.open('w')
                      genFilePath.encoding = 'BINARY'
                      genFilePath.write(waitToWrite)
                      genFilePath.close()
        ...
        if (currentXML.name() === 'Group') {
                ...
                for (shenqi = 0; shenqi < layerStyleArr.length; shenqi++) {
                  if (layerStyleArr[shenqi].indexOf('/') !== -1) {
                    if (layers.propertyDepth === 0) {
                      if (layers.containingComp.id === app.project.activeItem.id) { app.executeCommand(app.findMenuCommandId(cunName[shenqi])) }
          ...
          try {
            if (currentXML['@enabled'] !== 'None') {
              if (layers.property(propIndex).canSetEnabled === true) {
                if (prop === 0) {
                  if (currentXML['@enabled'] === 'false') { layers.property(propIndex).enabled = false }
                } else {
                  if (currentXML['@enabled'] === 'false') { layers.property(parseInt(prop.propertyIndex)).enabled = false }
                }
              }
            }
          } catch (err) { $.layer.errorInfoArr.push({ line: $.line, error: err }) }
          ...
          try {
            if (prop === 0) {
              ...
              if (layers.propertyType === PropertyType.INDEXED_GROUP) { layers.property(prop.propertyIndex).name = propName }
            }
          } catch (err) { $.layer.errorInfoArr.push({ line: $.line, error: err }) }
          ...
          for (ia = 0; ia < len; ia++) {
            ...
            try {
              ...
              if (type === PropertyValueType.TwoD_SPATIAL) {
                ...
                if (inSpatialArr.length === 3 && outSpatialArr.length === 3) { myScaleProperty.setSpatialTangentsAtKey(ia + 1, inSpatialArr, outSpatialArr) }
              }
            } catch (err) { $.layer.errorInfoArr.push({ line: $.line, error: err }) }
            try {
              ...
              try {
                if ((xml.child(ia + 4).isRoving).toString() === 'true') { myScaleProperty.setRovingAtKey(ia + 1, true) }
              } catch (err) { $.layer.errorInfoArr.push({ line: $.line, error: err }) }
            } catch (err) { $.layer.errorInfoArr.push({ line: $.line, error: err }) }
          }
        }
  ...
  $.layer.newProperties = function(effectxml, selectedLayers, options) {
    ...
    $.layer.forEach.call(selectedLayers, function(layer, index) {
      if (isKeyframeOffset === true) { $.layer.prototype.newPropertyGroup(effectxml, layer, layer.inPoint) } else { $.layer.prototype.newPropertyGroup(effectxml, layer) }
    })
    $.layer.correctProperty()
    $.layer.fixExpression()
    $.layer.setParent()
  ...
  $.layer.setParent = function() {
    $.layer.forEach.call($.layer.layerArr, function(item, index) {
      try {
        if (parseInt($.layer.layerParentNameArr[index]) === $.layer.layerParentNameArr[index]) { item.setParentWithJump(item.containingComp.layer(parseInt($.layer.layerParentNameArr[index]))) } else { item.setParentWithJump(item.containingComp.layer($.layer.layerParentNameArr[index])) }
      } catch (err) { }
    })
  }
...
; (function() {
  ...
  function colorPicker(inputColour, options) {
    if (!(this instanceof colorPicker)) { return new colorPicker(inputColour, options) }
  ...
  colorPicker.prototype.showColorPicker = function() {
    ...
    if (win.type === 'dialog' || win.type === 'palette') {
      if (this.haveSetting('location')) {
        ...
        if (win.location.length !== 2) { win.center() } else if (win.location[0] < 0 || win.location[1] < 0) { win.center() }
      }
      win.show()
  ...
  colorPicker.prototype.bindingHandler = function(win) {
    ...
    win.editor.colorHolder.colorCol1.rGroup.rValue.onChange =
      ...
      win.editor.colorHolder.colorCol3.bGroup.bValue.onChange = function() {
        ...
        if (this._index === 0) { win.editor.gulu.uni.unicode.text = colorPicker.RgbToHex([this.text / 255, _this.outputColour[1], _this.outputColour[2]]) } else if (this._index === 1) { win.editor.gulu.uni.unicode.text = colorPicker.RgbToHex([_this.outputColour[0], this.text / 255, _this.outputColour[2]]) } else if (this._index === 2) { win.editor.gulu.uni.unicode.text = colorPicker.RgbToHex([_this.outputColour[0], _this.outputColour[1], this.text / 255]) }
        win.editor.gulu.uni.unicode.notify('onChange')
      }
    ...
    win.editBright.onChange = win.editBright.onChanging = function() {
      ...
      if (isNaN(this.text) === true) { this.text = 100 }

      win.slider.value = parseInt(this.text)
      if (isNaN(win.slider.value)) { return }
  ...
  colorPicker.prototype.updateCursor = function(win) {
    if (colorPicker.arraysEqual(this.colourSelectCursor.strokeColour, [1, 1, 1])) {
      if (win.slider.value > 63) { this.colourSelectCursor.strokeColour = [0, 0, 0] }
    } else if (colorPicker.arraysEqual(this.colourSelectCursor.strokeColour, [0, 0, 0])) {
      if (win.slider.value <= 63) { this.colourSelectCursor.strokeColour = [1, 1, 1] }
    }
  }
  ...
  colorPicker.isRgb = function(rgbArr) {
    ...
    if (!colorPicker.isType(rgbArr, 'Array')) { return false }

    if (rgbArr.length !== 3) { return false }

    for (var i = 0, len = rgbArr.length; i < len; i++) {
      if (rgbArr[i] > 1 || rgbArr[i] < 0) { return false }
    }
  ...
  colorPicker.isLargeRgb = function(rgbArr) {
    ...
    for (var i = 0, len = rgbArr.length; i < len; i++) {
      if (rgbArr[i] > 255 || rgbArr[i] < 0) { return false }
    }
  ...
  colorPicker.isHex = function(hexStr) {
    ...
    if (!colorPicker.isType(hexStr, 'String')) { return false }
  ...
  colorPicker.isShortHex = function(hexStr) {
    ...
    if (hexStr.length !== 3) { return false }
  ...
  colorPicker.isHsb = function(hsbArr) {
    if (!hsbArr) { return false }

    if (!colorPicker.isType(hsbArr, 'Array')) { return false }

    if (hsbArr.length !== 4) { return false }
    ...
    listDraw: function() { // 重绘函数
      ...
      for (i = 0; i < items.length; i++) {
        if (items[i].image) {
          ...
          if (a > 0 && a - items[i].imageRect[3] < e.GV.size[1]) { g.drawImage(image, items[i].rect[0] + items[i].imageRect[0], items[i].rect[1] + items[i].imageRect[1] - e.scrollBarValue, items[i].imageRect[2], items[i].imageRect[3]) }
        }
      }
      ...
      for (i = 0; i < items.length; i++) {
        if (!items[i].selected) {
          ...
          if (a > 0 && a - items[i].imageRect[3] < e.GV.size[1]) { g.rectPath(items[i].rect[0], items[i].rect[1] - e.scrollBarValue, items[i].rect[2], items[i].rect[3]) }
        }
      }
      ...
      for (i = 0; i < items.length; i++) {
        if (items[i].strokeColor) {
          ...
          g.rectPath(items[i].rect[0], items[i].rect[1] - e.scrollBarValue, items[i].rect[2], items[i].rect[3])
          a = items[i].rect[1] + items[i].imageRect[1] - e.scrollBarValue + items[i].imageRect[3]
          if (a > 0 && a - items[i].imageRect[3] < e.GV.size[1]) { g.strokePath(pen) }
        }
      }
      // 为被选中的item描边
      for (i = 0; i < items.length; i++) {
      ...
      if (e.showText) {
        ...
        for (i = 0; i < items.length; i++) {
          if (items[i].strokeColor) {
            ...
            g.rectPath(items[i].rect[0] + items[i].fontRect[0], items[i].rect[1] + items[i].fontRect[1] - e.scrollBarValue, items[i].fontRect[2], items[i].fontRect[3])
            a = items[i].rect[1] + items[i].imageRect[1] - e.scrollBarValue + items[i].imageRect[3]
            if (a > 0 && a - items[i].imageRect[3] < e.GV.size[1]) { g.strokePath(brush) }
          }
        }
        // 为被选中的item文字添加背景色
        for (i = 0; i < items.length; i++) {
        ...
        for (i = 0; i < items.length; i++) {
          ...
          if (e.version === 'CC2014') { g.drawString(thisText, fontPen, items[i].rect[0] + items[i].fontRect[0], items[i].rect[1] + items[i].fontRect[1] - e.scrollBarValue - 10, font) } else { g.drawString(thisText, fontPen, items[i].rect[0] + items[i].fontRect[0], items[i].rect[1] + items[i].fontRect[1] - e.scrollBarValue, font) }
        }
      } // end of showText
    ...
    function Fns() {
      ...
      this.shortMenu = function(event) {
        ...
        if (event.button === 2 && event.detail === 1 && event.altKey === false) {
          ...
          if (currentPosition[0] + 180 > parseInt(finalPositionXString)) { currentPosition = [event.screenX - 180, event.screenY] }
